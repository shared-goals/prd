...
Из Текста выведены три чётких персонажа: Архитектор-предприниматель (Сергей), Городской волонтёр, Участник борьбы с зависимостью. Для MVP этого достаточно — три разных мотивации, три разных контекста использования.

Можно добавить Родителя (семейные цели) или Студента, но YAGNI: пока нет живых пользователей, добавление персонажей — это фикция. Рекомендую закрыть вопрос с текущими тремя и вернуться после первых 20 реальных пользователей.

**Нужно от Сергея:** согласие закрыть, или есть конкретный четвёртый персонаж которого он видит.

---

### Q2 — Главная метрика

Гипотеза платформы: человек в группе исполняет контракт лучше, чем один. Это нужно измерять с первого дня.

Предлагаю одну простую метрику: **доля исполненных контрактов в целях с ≥2 активными участниками vs личных контрактах**. Для MVP достаточно логировать флаг `is_solo` при каждом коммите — один булеан, ничего сложного. Порог "гипотеза подтверждена": разрыв ≥15%.

Это не требует решения прямо сейчас, но требует одной строки в схеме данных — иначе после запуска данных для анализа не будет. KISS: один флаг сейчас экономит месяц аналитики потом.

**Нужно от Сергея:** ок ли флаг `is_solo` в Commit, или есть другое видение метрики.

---

### Q10 — UX: показывать ли активность инициатора?

Из Goal Contagion research (раздел 10.14) следует: видеть что кто-то похожий уже начал — сильный триггер. Но конкретная форма не определена.

KISS-решение для MVP: в карточке цели показывать «Кто-то уже вложил X минут на этой неделе» (без имён, без сравнений). Один счётчик, считается по коммитам за 7 дней. Это не отдельная фича — это просто агрегат поверх уже существующих данных.

Вопрос скорее не «делать ли» (делать), а «в каком виде показывать при нуле активности». Рекомендую: при нуле — не показывать ничего, не писать "0 человек".

Это можно закрыть прямо сейчас — решение очевидное.

---

### Q12 — shared-goals skill для ИИ-агентов

Репозиторий github.com/shared-goals/skill упомянут в Тексте, но не описан. Это один из самых интересных векторов — интеграция с ИИ-помощниками (OpenClaw, Claude, ChatGPT).

YAGNI: для MVP v1 skill не нужен. Платформа должна сначала заработать как Telegram-бот. Skill — это API поверх работающей платформы, его нельзя спроектировать в вакууме.

Минимум что нужно сделать сейчас: зарезервировать репозиторий с README-заглушкой (4 операции: `find_goals`, `join_goal`, `commit`, `get_summary`). Протокол — MCP, он совместим с большинством агентов. Реализацию отложить на post-MVP.

**Нужно от Сергея:** репозиторий уже существует или нет? Если нет — создать заглушку или пропустить пока?

---

### Q13 — Позиционирование относительно государства

Текст говорит: Shared Goals — дополнительный инструмент, не замена государственным инициативам (Указ № 309). Это уже ответ.

YAGNI: вопрос партнёрства с муниципалитетами вообще не актуален для MVP. Пока нет продукта, не с чем идти к государству. Рекомендую отложить этот вопрос до появления первых реальных результатов метрик.

Позиция по умолчанию: независимый гражданский инструмент. Если появится интерес со стороны госструктур — рассмотреть тогда.
