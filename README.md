...
**Контекст:** Новая глава Текста (p2-999-death.md, черновик, коммит 2026-03-30) начинается с вопроса, который Шаг задал Сергею: стоит ли использовать понятие Веры в техническом задании на платформу? Сергей вынес его в публичное обсуждение — ответа пока нет.

**Суть вопроса:** Текст активно использует понятие Веры как один из четырёх психологических компонентов (Разум, Воля, Чувства, Вера). PRD в разделе 10.10 описывает критерии публичных целей через «евангельскую систему координат» с оговоркой «без религиозной привязки». Но является ли Вера частью модели данных или только частью Текста?

**Варианты:**
- **А.** Вера — только в Тексте (философское основание), в PRD не упоминается явно. Сохранить текущий статус.
- **Б.** Вера — в PRD как один из психологических механизмов Контракта, с академическими ссылками на commitment devices и spiritual vows (без конфессиональной окраски). Усилить раздел 10.7 (Контракт как духовный прецедент).
- **В.** Добавить поле `is_spiritual_vow: bool` в Контракт — для пользователей, для которых Контракт имеет духовное измерение. Показывать особую UI-метку «Зарок».

Предложение: **вариант Б** — обогащает PRD без изменения схемы данных. Вариант В — YAGNI до появления реальных пользователей данной категории.
