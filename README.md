...
- *Участник борьбы с зависимостями* — **✅ подтверждён из Текста (p2-180 #social_capital_fighting_addictions)**: Текст прямо описывает этот use case. Человек вступает в Общую цель «Борьба с недугом» через Зарок (=Контракт), следует Плану действий (=ActionPlan), делает ежедневные исполнения (=Commit). Работают все 4 психологических компонента: Разум, Воля, Чувства, [Вера](https://text.sharedgoals.ru/p2-120-school/#psychology_of_belief). Институт МИРВЧ использует схожую методику (духовно ориентированная стрессовая психотерапия с Зароком). Это не вырожденный случай — это полноценный персонаж платформы. **Добавлено v1.18:** Параллель Зарок/молитва подкреплена цитатой Антония Сурожского о молитве как «программе жизни» (p2-110-system.md, коммит 2026-03-26): Контракт в Shared Goals структурно идентичен духовному обету — добровольный, повторяемый, lifetime. [Источник](https://github.com/bongiozzo/whattodo/commit/55610e25a2b54a4df0ad9c6658148f27689301fa)
- *?* — **нужно уточнить у Сергея**: есть ли персонаж из корпоративной среды (Commitments → личные цели)?

**Уточнение (из Текста, p2-999-death.md, черновик):** Shared Goals работает для людей с совместимой системой координат — тех, для кого жизнь имеет ценность, слово имеет вес, и движение к цели важнее победы над другими. Платформа не претендует на универсальность: это не инструмент для всех, а инструмент для тех, кто уже спрашивает себя: «Что важно?»
...
*(Q2 закрыт в разделе 7: метрика принята, флаг `is_solo` добавлен в модель Commit)*
...
- **Нарушения духа платформы** (конкурентные цели, токсичный контент) — закрыто в разделе 10.10: AI-проверка при создании + форма модерации с обязательным примером нарушения + органическая фильтрация через happy moments.
...
- **Опциональный тег навыка** `skill_tag: Optional[Enum] = None` — какая из 4 областей особенно прокачалась:
  - `will` — Воля (преодоление, дисциплина, данное слово)
  - `mind` — Разум (изучение, анализ, планирование)
  - `feeling` — Чувства (творчество, эстетика, эмпатия)
  - `faith` — Вера (медитация, созерцание, духовная практика)
  
  Из Текста (p2-180, Контракт): «во время Исполнения можно уточнить, какие навыки особенно прокачались». Это не рейтинг навыков — это самодиагностика прогресса личности.
...
**Влияние на архитектуру MVP:** Для MVP — одна инстанция (Telegram-first). `instance_id + goal_id` = универсальный глобальный идентификатор цели. `instance_id: str = "default"` добавлен в модель Goal как заготовка — contract и commit наследуют instance через join.

**Решение принято (Вариант Б):** добавить `instance_id: str = "default"` только в модель `Goal`. Multi-tenancy логика не строится на MVP.
...
- Ожидаемое число пользователей MVP: **~20–50** (seed network: знакомые Сергея + кандидаты из группы «Шило»). [предположение, нужно подтвердить]
...
### Q — Goal Contagion UX: свежесть активности

✅ **Закрыт (2026-03-28).** Показывать свежесть отдельно от счётчика участников.

UX-правило:
- Есть коммиты за 7 дней → «Кто-то вложил X минут на этой неделе»
- Нет коммитов за 7 дней → только счётчик участников (без "тухлого" счётчика активности)
- 0 участников → «Будь первым»

**Основание (Текст, p2-180):** «тут ты видишь, что при приближении исполнения контракта твой товарищ, такой же занятый, готов его исполнять» — именно свежесть, а не факт существования цели, запускает goal contagion. Поэтому UX должен подчёркивать именно недавнюю активность.

**Схема данных:** не нужно нового поля — достаточно агрегата по `Commit.created_at` за 7 дней. Уже заложено в Q10 (v1.14).

---

### Q — Seed Network

[на подтверждение у Сергея]

**Кандидат из Текста:** Группа «Шило в Ж...» (p2-180) — реальное сообщество с совместными целями (походы, пробежки, активный отдых). Уже объединены психотипом Архитектор, ценят данное слово, имеют опыт совместного преодоления. Есть расширения: «Шило-Девочки» и «Детское Шило».

Альтернативный кандидат: участники борьбы с зависимостями (МИРВЧ, p2-180 #social_capital_fighting_addictions) — более сильная мотивация, но более деликатная аудитория.

**Вопрос к Сергею:** Готов ли предложить группе «Шило» попробовать платформу в качестве seed? Или нужна более анонимная/нейтральная группа для старта?
...
**Вопрос:** Должен ли создатель public-цели (не только глобальный администратор) иметь право удалить конкретный коммит участника, если тот нарушает дух цели (например, флуд или оффтоп в поле описания)?

**Варианты:**
- **А.** Нет — создатель не имеет власти над действиями других участников. Это принцип Shared Goals: нет иерархии, нет долженствования.
- **Б.** Да, с ограничением — только форматное удаление (спам/флуд), но не по содержанию. Решение логируется.
- **В.** Да, с апелляцией — участник может обжаловать удаление через систему модерации.
...
✅ **Закрыт (2026-03-26). Вариант Б принят.**

Добавить `instance_id: str = "default"` в модель `Goal`.

**Архитектурное обоснование:**
- `instance_id + goal_id` = универсальный глобальный идентификатор цели
- Goal — корневая сущность; Contract и Commit наследуют instance через join к Goal — менять их не нужно
- Архитектура инстанций в Тексте (p2-180) — планируемая, не гипотетическая
- Стоимость: одно поле с default-значением. Стоимость отсутствия: дорогая миграция с потерей консистентности позже

---

### Q_faith — Понятие «Вера» в PRD

✅ **Закрыт (2026-03-31). Вариант Б принят.**

**Обоснование из Текста (p2-999-death.md, черновик):**
Сергей прямо отвечает на вопрос Шага: «Веру потребуется включить в систему, чтобы на всех жизненных этапах, включая крайние, можно было говорить о Радости». Это не предположение — это авторский ответ. Вера включается как четвёртый навык, равноправный с Волей, Разумом и Чувствами.

**Что меняется в PRD:**
1. `skill_tag` enum в Commit расширен до четырёх значений включая `faith` (см. раздел 4.1)
2. Раздел 10.22 описывает системную роль всех четырёх навыков
3. Вера — психологический термин по Андрееву (Дух/Духовность), без конфессиональной привязки

**Вариант В** (поле `is_spiritual_vow`) — YAGNI до появления реальных пользователей данной категории. Оставить как возможный пост-MVP элемент.
...
**Обновлённое определение из Текста (p2-110-system.md, коммит 2026-03-30):**
> Любовь — навык отдавать (время, деньги, себя) и не ждать ничего взамен

Это разворачивает критерий в практическое правило: публичная цель должна описывать
отдачу — времени, знаний, усилий — без ожидания ответных действий от других.

✅ «Учусь и делюсь знаниями в открытом репозитории» — отдача без условий
❌ «Вместе читаем книгу каждую неделю» — содержит ожидание ответного участия
...
| | Цель | Тип |
|---|---|---|
| ✅ | «Бегаю каждое утро — фиксирую в Shared Goals» | Собственная практика, нет требований к другим |
| ❌ | «Все должны бегать каждый день» | Долженствование явное, требование к поведению других |
| ⚠️ | «Бегаем каждое утро вместе» | Пограничный случай — зависит от намерения |

Граница ⚠️-случая: если «вместе» описывает приглашение (я бегу, приходи если хочешь) — ✅. Если «вместе» означает условие (не буду бежать без тебя) — ❌.
...
**Культурный прецедент критерия №4 (из Текста, коммит 2026-03-26):**
Молитва как lifetime-программа (Антоний Сурожский, Силуан Афонский) — культурный прецедент для понимания что такое lifetime-цель. Это не «никогда не заканчивается» в смысле бесполезности — это «всегда актуально» в смысле бесконечного роста.
Примеры: «Быть в форме» = lifetime. «Пробежать марафон в мае» = event, не lifetime-цель Shared Goals.
...
*(Вопрос вынесен явно в раздел 7 — см. Q — Права создателя на модерацию коммитов)*
...
| 1.16 | 2026-03-26 | Q_new закрыт: instance_id добавлен в Goal (Вариант Б); instance_id + goal_id = универсальный глобальный идентификатор цели; Лукашев убран из тела PRD |
...
**✅ Закрыт (2026-03-28).** Показывать свежесть отдельно от счётчика участников (см. раздел 7 → Q Goal Contagion UX).
...
**⏳ Частично закрыт (2026-03-28).** Кандидат из Текста: группа «Шило в Ж...» (p2-180). На подтверждении у Сергея (см. раздел 7 → Q Seed Network).
...
### 10.20 Молитва как Контракт — духовный прецедент из Текста (добавлено v1.18, 2026-03-29)

**Из Текста (коммит 2026-03-26, p2-110-system.md):**

Сергей добавил цитату Антония Сурожского о молитве как «программе жизни»:
> «Выберите какую-нибудь молитву и сделайте её программой жизни — и вы увидите, что эта молитва никогда не наскучит, никогда не потускнеет, потому что изо дня в день она будет оттачиваться, сама жизнь будет придавать ей остроту.»

*— Антоний Сурожский, «Не могу, Господи, жить без Тебя»*

Это добавляет важный слой к пониманию механики Контракта в Shared Goals.

**Структурная параллель:**
| Молитва (Антоний Сурожский) | Контракт (Shared Goals) |
|---|---|
| Выбирается добровольно | Принимается добровольно |
| Повторяется регулярно | Commit — регулярные инвестиции |
| Наполняется смыслом через жизнь | Обогащается happy moments |
| Не наскучит при правильном подходе | Не становится бременем без долженствования |
| Программа жизни | Lifetime-цель |

**Значение для PRD:**
1. **Персонаж «Зависимости»** (раздел 1.3): Зарок в традиции Зарока/молитвы (МИРВЧ) и Контракт в Shared Goals имеют одну природу. Текст намеренно ставит в один ряд духовную практику и платформу. Это не случайное совпадение, а осознанная параллель автора.
2. **Компонент «Вера»** (p2-120-school.md, Силуан Афонский): Четвёртый компонент личности (по Андрееву) — Вера — не требует религиозного содержания. Это система координат, ориентир. Shared Goals может быть «программой жизни» для атеиста так же, как молитва для верующего.
3. **Критерий №4 публичных целей** («Могут происходить в течение всей жизни») — теперь подкреплён не только продуктовой логикой, но и культурным прецедентом: lifetime commitment — универсальная духовная практика разных традиций.

**Практический вывод для PRD:**
Тезис «Shared Goals — это lifetime commitment, а не task tracker» имеет глубокое культурное обоснование через Текст. При описании платформы (в FAQ, онбординге) уместно апеллировать к этой параллели.

**Источники:**
- [Коммит Anthony of Sourozh](https://github.com/bongiozzo/whattodo/commit/55610e25a2b54a4df0ad9c6658148f27689301fa)
- [Коммит Silouan of Athos](https://github.com/bongiozzo/whattodo/commit/ac9427eece3319fb41e3cc42384bf392e1cba3bc)

---

### 10.21 Memento Mori — системы координат и границы платформы (добавлено v1.20, 2026-03-31)

**Из Текста (p2-999-death.md, черновик, коммит 2026-03-30):**
[https://github.com/bongiozzo/whattodo/commit/47472ebbafe42674df4c2c988d7939b0d0dab8c1]

Сергей формулирует ключевой принцип: «Если системы координат разные — сходимости не добиться никогда.»

Для Shared Goals это означает:
- Платформа не универсальна. Она работает для людей, у которых достаточно близкая система координат — прежде всего в вопросах ценностей и отношения к жизни (не обязательно одинаковая, но совместимая).
- Критерии публичных целей (гуманистичность, отсутствие долженствования, Любовь как вектор) — это не просто модерация контента, это фильтр совместимости систем координат участников.
- Аудитория платформы на MVP — люди, для которых «данное слово весомо» и которые ищут сонаправленность без принуждения. Это более узкое определение, чем «все люди с целями».

**Следствие для shared-goals skill:**
ИИ-агент в роли помощника может выступать «переводчиком» между близкими системами координат — помогать участнику сформулировать цель так, чтобы найти людей, идущих в том же направлении, даже если они используют другие слова. Это обоснование операции `find_goals` как семантического поиска, а не только текстового.

**Статус Текста:** p2-999-death.md — черновик («Незавершённая глава»). PRD отражает черновой тезис; при публикации финальной версии — обновить ссылку.

### 10.22 Четыре навыка как ось развития личности в Shared Goals (добавлено v1.21, 2026-03-31)

**Из Текста (p2-120-school.md, p2-180, p2-999-death.md):**

Текст строит модель личности на четырёх психологических областях по Андрееву:

| Навык | Термин в Тексте | Суть |
|---|---|---|
| Воля | `will` | Преодоление, данное слово, дисциплина движения к цели |
| Разум | `mind` | Анализ, планирование, изучение — рациональное начало |
| Чувства | `feeling` | Творчество, эстетика, эмпатия — иррациональное начало |
| Вера | `faith` | Система координат, ориентир, духовное измерение жизни |

Это не религиозная типология — это психологическая модель из работ Владимира Андреева, используемая в Тексте как практический инструмент. Вера в этом контексте = Дух, внутренний ориентир, то что задаёт направление когда Разум молчит.

**Почему все четыре важны для Shared Goals:**

Из p2-180 (Контракт): «соотношение требуемых навыков для Контракта очень индивидуально. Для кого-то физические нагрузки требуют повышенного включения Воли, другим эти занятия изначально даются легко. То же самое с каждой из областей нашего мироощущения».

Из p2-999-death.md: «Веру потребуется включить в систему, чтобы на всех жизненных этапах, включая крайние, можно было говорить о Радости». Без Веры платформа описывает только срединный путь жизни — и теряет целостность.

**Как четыре навыка реализованы в платформе:**

- **В Цели** — критерии гуманистичности (10.10) косвенно фильтруют через все четыре измерения: Разум (благородное любопытство), Воля (не долженствование), Чувства (Любовь как вектор), Вера (Цели больше жизни)
- **В Контракте** — тип контракта отражает доминирующий навык: ежедневная пробежка = Воля, изучение языка = Разум, рисование = Чувства, медитация/созерцание = Вера
- **В Коммите** — `skill_tag` (will/mind/feeling/faith) позволяет участнику отметить что именно прокачалось в этом исполнении
- **В Агрегатах** — суммарное распределение тегов по цели даёт картину «навыкового профиля» цели (post-MVP)

**Практическое следствие для дизайна:**

`skill_tag` — это не оценка, не рейтинг, не обязательное поле. Это **зеркало**: «что происходит со мной, когда я движусь к этой цели?». Заполняется по желанию. Агрегируется анонимно. Не сравнивает людей между собой.

**Вера как четвёртый навык — особый случай:**

Из Текста прямо: «для развития навыков Веры необходимы медитативные практики — молитва, созерцательное пребывание в первозданной Природе, вероятно, любое действие, которое вызывает высокотональное состояние радости, близкое к благоговению».

Это значит, что `faith` в `skill_tag` — это не «религиозная практика». Это любое действие, которое приводит к состоянию благоговения. Поход на Эльбрус может дать `faith` так же как молитва. Это важно для UI: никакой религиозной символики, просто звёздочка или иконка созерцания.

**Источники:**
- [p2-120-school.md #types_of_psychology](https://text.sharedgoals.ru/p2-120-school/#types_of_psychology)
- [p2-180-sharedgoals.md #entity_contract](https://text.sharedgoals.ru/p2-180-sharedgoals/#entity_contract)
- [p2-999-death.md #sg_skills](https://github.com/bongiozzo/whattodo/blob/master/text/p2-999-death.md#sg_skills) (черновик)
...
### v1.22 — 2026-04-01
- **Коммит "docs: add draft of memento mori chapter" (2026-03-30):** уточнение критерия №3 «Любовь как вектор» (раздел 10.10) — обновлённое определение из p2-110-system.md («Любовь — навык отдавать»), канонический пример ✅/❌/⚠️ (бегать каждый день vs требование к другим), подтверждение фильтра совместимости систем координат из p2-100-authors.md

### v1.21 — 2026-03-31
- **Q_faith закрыт:** Вариант Б принят — Вера включается как четвёртый навык, равноправный с Волей, Разумом и Чувствами. Обоснование из p2-999-death.md (авторский ответ на вопрос Шага)
- **Раздел 4.1 (Commit):** добавлен `skill_tag: Optional[Enum]` с вариантами `will / mind / feeling / faith`
- **Раздел 10.22:** «Четыре навыка как ось развития личности в Shared Goals» — системное описание модели, практическая реализация в платформе, особый случай Веры в UI

### v1.20 — 2026-03-31
- **Коммит whattodo 2026-03-30 (p2-999-death.md, черновик «Memento Mori»):**
- Добавлен раздел **10.21** — Memento Mori: системы координат и границы платформы
- Уточнена **1.3** (целевая аудитория) — тезис о совместимости систем координат
- Добавлен открытый вопрос **Q_faith** — понятие «Вера» в PRD (вариант Б на рассмотрении)

### v1.18 — 2026-03-29
- **Коммиты whattodo 2026-03-26 (Антоний Сурожский, Силуан Афонский):**
- Добавлен раздел **10.20** — Молитва как Контракт (духовный прецедент из Текста)
- Обновлён персонаж зависимостей **(1.3)** — параллель Зарок/молитва/Контракт с цитатой Антония Сурожского
- Критерий №4 модерации **(10.10)** подкреплён культурным прецедентом (lifetime commitment как духовная практика)

### v1.17 — 2026-03-28
- **Q Goal Contagion UX закрыт:** UX-правило свежести 7 дней (раздел 7)
- **Q Seed Network добавлен:** кандидат «Шило в Ж...» зафиксирован, вопрос к Сергею (раздел 7)
- 10.14: ❓ закрыт → ✅
- 10.15: ❓ частично закрыт → ⏳
- Коммиты 2026-03-26 (Антоний Сурожский, Силуан Афонский): духовно-философские, продуктовых изменений нет
