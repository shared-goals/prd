...
### Предложения по открытым вопросам

**Q1 — Персонажи:**
**Предложение:** Из Текста уже выведены: (A) Архитектор-предприниматель (Сергей), (B) Городской волонтёр, (C) Участник борьбы с зависимостью. Возможные дополнительные: (D) Родитель с детьми (семейные цели — совместные прогулки, чтение), (E) Студент (учёба, спорт с однокурсниками). Предложить Сергею выбрать 1-2 из D/E для включения в MVP-персонажей.

**Q2 — Главная метрика:**
**Предложение:** Зафиксировать как основную метрику: «contract execution rate в целях ≥2 активных участников vs личные контракты». Порог подтверждения гипотезы: разрыв ≥15%. Для MVP необходимо логировать `is_solo` при каждом коммите с первого дня. [на подтверждение у Сергея]

**Q6 — Ожидаемый масштаб MVP:**
**Предложение:** Варианты: (A) Минимальный — 50-100 пользователей, атомарная сеть 1-2 районов; (B) Умеренный — 300-500 пользователей, 3-5 городов; (C) Не ограничивать заранее, строить на SQLite до упора (~10k коммитов), потом мигрировать на PostgreSQL. Рекомендация: вариант C — не планировать масштаб, строить инфраструктуру с возможностью роста.

**Q7 — Аналоги:**
**Предложение:** Основные аналоги уже разобраны в 10.1. Вывод: прямых аналогов нет. Ближайшие — StickK/Beeminder (commitment devices с денежными штрафами), но без социальной наблюдаемости без санкций. Можно считать вопрос закрытым — конкурентная ниша свободна. [на подтверждение у Сергея]

**Q10 — UX-паттерн «инициатор виден»:**
**Предложение:** Показывать в карточке цели: «Кто-то уже инвестировал X минут на этой неделе» (без имени, без сравнений). Это минимальное изменение UX с максимальным goal contagion эффектом. Реализовать в MVP как простой счётчик «активность за последние 7 дней».

**Q11 — Cold start:**
**Предложение:** Стратегия из трёх шагов: (1) Сергей создаёт 3-5 seed goals вручную; (2) Приглашает 10-20 людей из личного окружения через invite-ссылки; (3) Публичный онбординг открывается только когда seed goals показывают ≥3 активных участника. Не требует изменений в продукте — только операционный план запуска.

**Q12 — shared-goals skill:**
**Предложение:** Минимальный набор операций навыка: `find_goals` (поиск по ключевым словам), `join_goal` (присоединение + создание контракта), `commit` (фиксация исполнения), `get_summary` (сводка по целям пользователя). Протокол: MCP-сервер (совместим с Claude, OpenAI Agents и OpenClaw). Репозиторий: создать как заглушку с README на основе этих операций. [нужно подтвердить у Сергея]

**Q13 — Позиционирование относительно государства:**
**Предложение:** Варианты: (A) Независимый гражданский инструмент — без партнёрств, но с нарративом «дополняем нацели снизу вверх»; (B) Открыт к пилотам с отдельными муниципалитетами (Цифровой Петербург, «Твой Бюджет»); (C) Сначала независимо, потом рассмотреть партнёрство после MVP. Рекомендация: вариант C — сначала продукт, потом позиционирование. [на подтверждение у Сергея]
